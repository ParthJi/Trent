...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(state: Bundle?) {
        ...
        web.webChromeClient = object : WebChromeClient() {
            override fun onShowFileChooser(v: WebView?, cb: ValueCallback<Array<Uri>>?, p: FileChooserParams?): Boolean {
                ...
                return try { startActivityForResult(p!!.createIntent(), REQ); true } catch (_: Exception) { chooser = null; false }
            }
    ...
    private fun requestRuntimePermissions() {
        ...
        if (Build.VERSION.SDK_INT >= 33) { list += Manifest.permission.READ_MEDIA_IMAGES; list += Manifest.permission.READ_MEDIA_VIDEO }
        if (Build.VERSION.SDK_INT >= 23 && ContextCompat.checkSelfPermission(this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) list += Manifest.permission.CAMERA
        ...
        @JavascriptInterface fun saveFile(name: String, mime: String, base64: String) {
            try {
                ...
                if (Build.VERSION.SDK_INT >= 29) {
                    val values = ContentValues().apply { put(MediaStore.Downloads.DISPLAY_NAME, safe); put(MediaStore.Downloads.MIME_TYPE, mime); put(MediaStore.Downloads.IS_PENDING, 1) }
                    ...
                    val dir = getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS)!!; dir.mkdirs(); FileOutputStream(File(dir, safe)).use { it.write(bytes) }
            ...
            } catch (e: Exception) { runOnUiThread { Toast.makeText(this@MainActivity, "Export failed: ${e.message}", Toast.LENGTH_LONG).show() } }
        }
        @JavascriptInterface fun shareFile(name: String, mime: String, base64: String) {
            try {
                val bytes = Base64.getDecoder().decode(base64); val file = cacheForShare(name, mime, bytes)
            ...
            } catch (e: Exception) { runOnUiThread { Toast.makeText(this@MainActivity, "Share failed: ${e.message}", Toast.LENGTH_LONG).show() } }
        }
        private fun cacheForShare(name: String, mime: String, bytes: ByteArray): File {
            val dir = File(cacheDir, "shared"); dir.mkdirs(); val f = File(dir, name.replace(Regex("[^A-Za-z0-9._-]"), "_")); FileOutputStream(f).use { it.write(bytes) }; return f
        }
    }
}
